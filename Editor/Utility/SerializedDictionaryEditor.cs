...
namespace L11.Editor.Utility
{
    [CustomPropertyDrawer(typeof(SerializedDictionaryDrawable), useForChildren: true)]
    public class SerialziedDictionaryEditor : PropertyDrawer
    {
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            {
                height += CalculateElementHeight(values.GetArrayElementAtIndex(i), state == null ? 0 : state.GetWarnings(i).Count);
        ...
        {
            var list = new ReorderableList(property.serializedObject, property.FindPropertyRelative("_keys"));
        ...
        private static float OnCalculateElementHeight(SerializedProperty property, int index, int warnings)
        {
            ...
            return CalculateElementHeight(property.FindPropertyRelative("_values").GetArrayElementAtIndex(index), warnings);
        }

        private static float CalculateElementHeight(SerializedProperty elementProperty, int warnings)
        {
            var result =
                EditorGUIUtility.standardVerticalSpacing * 2 +
                EditorGUIUtility.singleLineHeight +
                EditorGUI.GetPropertyHeight(elementProperty, includeChildren: true);
        ...
        private static void OnDrawListElement(SerializedProperty property, Rect rect, int index, bool active, bool focused, List<Warning> warnings)
        {
            ...
            var offset = EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing * 2;
            valueRect.y += offset;
